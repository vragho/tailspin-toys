...
- Document a reusable component's public contract in its `Props` interface so the API is clear to other contributors and to Copilot.
...
---
/**
 * Renders a compact list item for a title and supporting summary.
 */
interface Props {
  title: string;
  summary?: string;
